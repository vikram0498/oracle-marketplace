...
class CatalogController extends FrontBaseController
{
    ...
    //   dd(session::get('view'));
      ...
      if (!empty($slug3)) {
        $subchildcat = Subchildcategory::where('slug', $slug3)->firstOrFail();
        $data['subchildcat'] = $subchildcat;
      }


      if (!empty($slug4)) {
        $childchildcat = Childchildcategory::where('slug', $slug4)->firstOrFail();
        $data['childchildcat'] = $childchildcat;
      }
