...
@section('content')
					<input type="hidden" id="headerdata" value="{{ __('CHILD 3 CATEGORY') }}">
<input type="hidden" id="attribute_data" value="{{ __('ADD NEW ATTRIBUTE') }}">
					<div class="content-area">
						<div class="mr-breadcrumb">
							<div class="row">
								<div class="col-lg-12">
										<h4 class="heading">{{ __('Child 3 Categories') }}</h4>
										<ul class="links">
											<li>
											...
											<li>
												<a href="{{ route('admin-childchildcat-index') }}">{{ __('Child 3 Categories') }}</a>
											</li>
										</ul>
								</div>
							</div>
...
{{-- DATA TABLE --}}
    ...
    <script type="text/javascript">
		...
		var table = $('#geniustable').DataTable({
               ...
               processing: true,
               serverSide: true,
               ajax: '{{ route('admin-childchildcat-datatables') }}',
               columns: [
               			{ data: 'category', searchable: false, orderable: false},
               			{ data: 'subcategory', searchable: false, orderable: false},
               			{ data: 'childcategory', searchable: false, orderable: false},
               			{ data: 'subchildcategory', searchable: false, orderable: false},
                    { data: 'name', name: 'name' },
										{ data: 'slug', name: 'slug' },
        ...
        $(".btn-area").append('<div class="col-sm-4 table-contents">'+
        	'<a class="add-btn" data-href="{{route('admin-childchildcat-create')}}" id="add-data" data-toggle="modal" data-target="#modal1">'+
