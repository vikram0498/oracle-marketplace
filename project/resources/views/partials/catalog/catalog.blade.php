                            ...
                            <li class="cat-item cat-parent">
                                ...
                                @if($subcategory->childs->count()!=0)
                                    ...
                                    <ul class="children">
                                        ...
                                        <li class="cat-item ">
                                            <a href="{{route('front.category', [$category->slug, $subcategory->slug, $childelement->slug])}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}" class="category-link {{ isset($childcat) ? ($childcat->id == $childelement->id ? 'active' : '') : '' }}"> {{$childelement->name}} <span class="count"></span>
                                            </a>

                                            @if($childelement->subchilds->count() > 0)
                                            <span class="has-child"></span>
                                            <ul class="children">
                                                @foreach (App\Models\Subchildcategory::where('childcategory_id',$childelement->id)->get() as $key => $subchildelement)
                                                <li class="cat-item ">
                                                    <a href="{{route('front.category', [$category->slug, $subcategory->slug, $childelement->slug,$subchildelement->slug])}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}" class="category-link {{ isset($subchildcat) ? ($subchildcat->id == $subchildelement->id ? 'active' : '') : '' }}"> {{$subchildelement->name}} <span class="count"></span>
                                                    </a>

                                                    @if($subchildelement->childchildcategory->count()!=0)
                                                    <span class="has-child"></span>
                                                    <ul class="children">
                                                         @foreach (DB::table('childchildcategories')->where('subchildcategory_id',$subchildelement->id)->get() as $key => $childchildelement)
                                                         <li class="cat-item ">
                                                            <a href="{{route('front.category', [$category->slug, $subcategory->slug, $childelement->slug,$subchildelement->slug,$childchildelement->slug])}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}" class="category-link {{ isset($childchildcat) ? ($childchildcat->id == $childchildelement->id ? 'active' : '') : '' }}"> {{$childchildelement->name}} <span class="count"></span>
                                                            </a>
                                                         </li>
                                                         @endforeach
                                                    </ul>
